...
12. Author-Defined Terms, Public Contracts, and Verification Boundaries

13. Conclusion: AI Needs Verifiable Physical Memory, Not Merely More Numbers
...
ASNS, vCAN, EDC, Tagger, and CID are author-defined technical terms used by Embodied Worker Co., Ltd. and Kangaroo TEC Corporation to describe this architecture and its current implementation. They are not existing international standards. This paper publishes their engineering roles, minimum evidence requirements, verification conditions, and responsibility boundaries while withholding internal frame formats, firmware, algorithms, source code, and other proprietary implementation details.
...
Within the currently verified standard implementation scope, L1-S includes read-only Modbus TCP mirrors and Modbus RTU data converted through an approved gateway that preserves source lineage. This is a present source-admission and product-implementation boundary; it does not imply that Modbus is inherently evidentiary. L1-S retains the value supplied by the source system at that time and does not claim to be the sensor’s lowest-level ADC code. MQTT, OPC UA, BACnet, or arbitrary PLC points likewise do not acquire L1 evidentiary status merely because they can be connected. Any additional source path requires an explicit adapter specification, source-boundary definition, timestamp semantics, acquisition-state mapping, configuration version, and deployment verification.
...
This section states the status of the author-defined technical terms used in this paper, their public responsibilities, the distinction between architecture and product implementation, and the minimum conditions that an external reviewer can examine. Its purpose is not to present proprietary names as industry standards. Its purpose is to make the proposal clear, challengeable, and verifiable without requiring disclosure of implementation secrets.

## 12.1 Status of the Terms

ASNS, vCAN, EDC, Tagger, and CID are author-defined technical terms introduced or used by Embodied Worker Co., Ltd. and Kangaroo TEC Corporation. They are not existing international standards, and public publication does not automatically make them open standards.

- ASNS denotes the overall sensing, evidence, and knowledge architecture.

- vCAN denotes an identity-aware sensing communication protocol and data language implemented on CAN 2.0B.

- EDC denotes the edge data core that preserves and serves L1 time-series evidence.

- Tagger denotes the engineering workspace for replay, annotation, baseline formation, and knowledge review.

- CID denotes the stable identity that maintains continuous reference to a data channel across time, names, versions, and systems.

These names are published to establish a common vocabulary that can be cited and examined. Publishing the architectural definition does not disclose every implementation method and does not grant rights to reproduce undisclosed protocols, software, firmware, or algorithms.

## 12.2 Public Responsibilities of the Components

| Name and technical type | Public responsibility | Explicitly outside its role |
|---|---|---|
| ASNS: overall architecture | Establishes a traceable chain from physical sensing to evidence and reviewed knowledge | Does not replace PLC, SCADA, SIS, or deterministic control |
| vCAN: sensing communication protocol | Maintains device and channel identity, common timing basis, configuration context, and acquisition state | Does not perform AI inference, reporting, or equipment control |
| EDC: edge data core | Preserves L1 records and source lineage and provides read-only retrieval by CID and time range | Is not an AI model, SCADA system, or equipment controller |
| Tagger: engineering workspace | Supports replay, human annotation, golden baselines, tasks, and reviewed knowledge formation | Does not independently declare equipment failure or replace accountable engineers |
| CID: channel identity mechanism | Maintains continuous reference to a data channel when names, configurations, or systems change | Is not a display tag, equipment name, or database field name |
| L1/L2/L3: governance layers | Separate original acquisition, recalculable results, and reviewed knowledge | Do not imply that all three layers have equal evidentiary or responsibility status |

## 12.3 Architectural Claims and Current Implementations Must Remain Distinct

ASNS is the architectural proposition defined by this paper. vCAN, EDC, and Tagger are the current technical components used by Kangaroo TEC Corporation and Embodied Worker Co., Ltd. to implement that architecture. They are not claimed to be the only possible implementation of every evidence-first principle.

An external system may adopt similar Evidence-First AI principles without identifying itself as an ASNS implementation. Conversely, merely using CAN, Modbus, an edge database, a time-series database, or an annotation tool does not demonstrate conformance with ASNS. Alignment depends on whether the complete system can demonstrate a continuous chain of channel identity, explicit time semantics, acquisition state, source lineage, L1 preservation, versioned computation, human review, and historical replay.

## 12.4 Minimum Publicly Verifiable Conditions

A deployment claiming conformance or alignment with this public ASNS definition should be able to demonstrate at least the following:

1. Every evidence channel has a stable and traceable CID.

2. Timestamp semantics are explicitly declared, and reception time or storage time is not misrepresented as native device event time.

3. Acquisition state is preserved with the value, and communication failure is not concealed as a normal value or unexplained null.

4. Source type and configuration version remain traceable.

5. Supported user interfaces and APIs provide no normal path for overwriting historical L1 records.

6. L2 retains its rules, parameters, version, and referenced L1 intervals so that results can be recalculated.

7. L3 knowledge is human-reviewed and retains the reviewer, version, applicability conditions, and evidence references.

8. AI conclusions can identify the CIDs, time interval, data granularity, L2 version, and knowledge sources used so that people can retrieve, replay, and verify the evidence.

Meeting these conditions does not mean that sensors are always correct or that AI judgments are necessarily correct. It means that the system provides the evidence conditions required for errors to be discovered, traced, recalculated, and corrected.

## 12.5 Public and Non-Public Scope

This paper publishes architectural roles, data-layer boundaries, the minimum L1 record, time semantics, acquisition-state principles, source lineage, replay requirements, AI control boundaries, public verification conditions, and technical responsibility boundaries.

This paper does not publish the complete vCAN frame format, address-coordination details, firmware, source code, internal time-synchronization algorithms, EDC internal write and storage mechanisms, core L2 algorithms and parameters, security keys, deployment credentials, customer configurations, manufacturing data, or other undisclosed commercial and technical information.

This disclosure boundary allows external readers to evaluate the architectural claims without requiring the rights holders to release enough implementation detail to reproduce the products. Publishing technical responsibilities and verification conditions does not waive intellectual-property rights.
...
ASNS, vCAN, EDC, Tagger, and CID are author-defined technical terms introduced or used by the author and the associated technical team; they are not existing international standards. This paper publicly describes their architectural principles, public contracts, and verification boundaries. It is not a complete communication specification, product specification, performance warranty, or safety-control design. Actual functions and sampling capabilities are governed by official product specifications, deployment design, and verification results. Internal vCAN frame formats, algorithms, firmware, and undisclosed implementation details are outside the scope of this paper.
